...
- (id)initWithAvItem:(CGUpnpAvItem*)anAvItem
{
    ...
    if (self) {
        ...
        TestUpnpAppDelegate* app = [[UIApplication sharedApplication] delegate];
...
//    [coverArtView release];
    ...
    NSLog(@"%@", self.avItem);
        NSLog(@"%@", self.avItem.resourceUrl);
        NSLog(@"%@", [self.avItem.resourceUrl description]);
    NSLog(@"%@", [self.renderer friendlyName]);
    //if ([self.renderer setAVTransportURI:[self.avItem.resourceUrl description]]) {
    //if ([self.renderer setAVTransportURI:@"http://cdn.udn.com/photo/web/news/20140322/98957_58bfc9882581d876_l.jpg"]){
    if ([self.renderer setAVTransportURI:@"http://cdn.udn.com/vods3/20140322151016696@500_1500/playlist.m3u8"]){
        [self.renderer play];
